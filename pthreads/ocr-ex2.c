...
void* stageThreshold(void *arg)
{
	...
	while (readers > 0)
	{
	    /* wait until an image becomes available */
    	...
    	/* copy the image to the local buffer and free the original */
...
int main(int argc, char **argv)
{
	...
	tid = 0;
	tids = malloc(sizeof(pthread_t) * (readers + thresholders + segmenters +
				correlators + outputs));
	for (i=0; i<readers; ++i)
	    pthread_create(&tids[tid++], NULL, stageRead, &argc);
	for (i=0; i<thresholders; ++i)
	    pthread_create(&tids[tid++], NULL, stageThreshold, NULL);
	for (i=0; i<segmenters; ++i)
	    pthread_create(&tids[tid++], NULL, stageSegmentation, NULL);
	for (i=0; i<correlators; ++i)
	    pthread_create(&tids[tid++], NULL, stageCorrelation, NULL);
	for (i=0; i<outputs; ++i)
	    pthread_create(&tids[tid++], NULL, stageOutput, NULL);
